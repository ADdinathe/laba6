...
static const string ISEAL("0000");
static const int SIZE_ROTATION (5*1024*1024);
static const int ZERO (0);
static const string WAY ("../logs/logFile_%N.log");
static const string FORMAT ("[%TimeStamp%]: %Message%");
void init_logs() {
    auto log = boost::log::add_file_log(
            boost::log::keywords::file_name = WAY,
            boost::log::keywords::rotation_size = SIZE_ROTATION,
            boost::log::keywords::time_based_rotation = boost::log::sinks::file::rotation_at_time_point{ ZERO, ZERO, ZERO },
...
void logs() {
    ...
    while (true) {
        ...
        if (Crypted.substr(Crypted.size() - ISEAL.size()) != ISEAL)
